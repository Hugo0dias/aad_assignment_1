...
#define N_LANES 16            // AVX-512: 512 bits / 32 bits = 16 lanes
#define LANE_SIZE 56
...
typedef struct {
    __m512i chunks[NONCE_CHUNKS * 2];  // 2 porque cada __m512i tem 8 lanes de 64 bits e sao 16 lanes no total
...
//static inline void increment_nonces16(nonce16_t *nonce) {
//    // incrementa primeiro chunk (LSB)
//    __m512i one = _mm512_set1_epi64(1);
//    __m512i prev0 = nonce->chunks[0];              // lanes 0..7
//    __m512i prev1 = nonce->chunks[1];              // lanes 8..15
//
//    __m512i added0 = _mm512_add_epi64(prev0, one);
//    __m512i added1 = _mm512_add_epi64(prev1, one);
//
//    // store back
//    nonce->chunks[0] = added0;
//    nonce->chunks[1] = added1;
//
//    // gera carry por lane (0 ou 1)
//    uint64_t carry_lo = 0, carry_hi = 0;
//    uint64_t *p0 = (uint64_t*)&prev0;
//    uint64_t *a0 = (uint64_t*)&added0;
//    for (int i = 0; i < 8; i++)
//        carry_lo |= ((a0[i] < p0[i]) ? (1ULL << i) : 0);
//    uint64_t *p1 = (uint64_t*)&prev1;
//    uint64_t *a1 = (uint64_t*)&added1;
//    for (int i = 0; i < 8; i++)
//        carry_hi |= ((a1[i] < p1[i]) ? (1ULL << i) : 0);
//
//    // propagação de carry scalar por lane para os chunks seguintes
//    for (int c = 1; c < NONCE_CHUNKS; c++) {
//        for (int i = 0; i < 8; i++) {
//            uint64_t val = ((c % 2) ? ((uint64_t*)&nonce->chunks[c])[i] : ((uint64_t*)&nonce->chunks[c])[i+8]);
//            if (c % 2 == 0) {
//                if (carry_lo & (1ULL << i)) val += 1;
//            } else {
//                if (carry_hi & (1ULL << i)) val += 1;
//            }
//            if (c % 2 == 0)
//                ((uint64_t*)&nonce->chunks[c])[i] = val;
//            else
//                ((uint64_t*)&nonce->chunks[c])[i+8] = val;
//        }
//        // recompute carry_lo/carry_hi for next chunk if needed
//        // omitido aqui para simplicidade, normalmente é raro ter carry > 1 chunk
//    }
//
//    // incrementa bytes restantes scalarmente
//    for (int lane = 0; lane < N_LANES; lane++) {
//        if ((nonce->chunks[NONCE_CHUNKS-1][lane] == 0ULL)) {
//            nonce->rem[lane]++;
//        }
//    }
//}
...
int main(void) {
    ...
    uint8_t msg[N_LANES * LANE_SIZE] __attribute__((aligned(64))); // alinhado para AVX-512
    v16si coin[14];   // 14 words, cada uma com 16 lanes de 32 bits
    v16si hash[5];
    ...
    while (1) {
        ...
        // chama SHA1 AVX-512 (deve existir na tua lib aad_sha1_cpu.h)
